...
当前项目属于3x-ui 二开版本, 所有二开特性在 FEATURE.md 中. 合并上游代码前注意查看 FEATURE.md 中的二开特性, 避免被覆盖.
当完成一个功能后, 请在 FEATURE.md 中添加该功能的描述, 以便下次合并上游代码时检查是否被覆盖.
描述规范:
1. 必须是有价值的功能, 不能是无意义的修改描述,例如: "新增了用户管理功能" 而不是 "修改了README 文档 / 更新 CI 工作流".
2. 描述应简洁明了, 突出功能的核心价值和使用场景.

二开注意事项:
1. 避免直接修改上游代码, 尽量通过继承或组合的方式实现功能扩展, 以便于后续合并上游代码时减少冲突.
2. 在二开过程中, 如果需要修改上游代码, 请务必在 FEATURE.md 中记录修改的原因和目的, 以便后续维护和合并上游代码时参考.
3. 如果产生了合并冲突, 需要以减少后续再次冲突为原则, 进行合理的代码调整和重构, 并在 FEATURE.md 中记录冲突解决方案和修改内容.
