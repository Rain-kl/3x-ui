...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/Rain-kl/3x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/Rain-kl/3x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/Rain-kl/3x-ui/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/Rain-kl/3x-ui/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/Rain-kl/3x-ui/main/x-ui.sh"
    fi
}
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/Rain-kl/3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/Rain-kl/3x-ui/main/install.sh)${plain}"
    echo ""
